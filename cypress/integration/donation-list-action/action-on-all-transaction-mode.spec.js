...
describe("All Transactions", () => {
  ...
  let testData;
  beforeEach(function () {
    cy.fixture('donation-list-action-data/all-transactions-data').then(function (data) {
      testData = data;
      return testData;
    });
  })
  it('action on all transactions', () => {
    ...
    cy.log("**transaction_type**" + transaction_type);
    ...
    if (transaction_type == 'cash') {
      ...
      if (typeof testData.amount == 'number') {
        ...
        cy.wrap(3).should('eq', 4, { message: 'Amount is more than 2000 or not in number formate' });
      }
      ...
      let random_utr_no = Math.floor(1000000000 + Math.random() * 9000000000);
      let UTR_No = random_utr_no;
      ...
      if (test_same_digit(UTR_No)) {
        cy.wrap(5).should('eq', 6, { message: 'UTR number digits are same' });
      }

      function UTR_digits_count(n) {
        var count = 0;
        if (n >= 1) ++count;
      ...
      if (UTR_digits_count(UTR_No) >= 10) {
        cy.get('#mat-input-16').type(UTR_No);
      }
      else {
        cy.wrap(1).should('eq', 2, { message: 'UTR_NO is not of 10 digits or not in number format' });
      }
      ...
      if (testData.category == 'individual') {
        ...
        // if proprietorship is yes
        if (testData.proprietorship != 'false') {
          cy.get('[formcontrolname="proprietorship_name"]').type(testData.proprietorship_name);
      ...
      if (testData.flag_operation == 'Edit') {
        ...
        cy.wait(1000).then(() => {
          ...
          if (testData.flag_amount != 0) {
            // verify amount format and rearrange it to 2 digits after decimal if any
      ...
      let cheque_no = Math.floor(100000 + Math.random() * 900000);
      cy.log('**cheque no:**' + cheque_no);
      if (test_same_digit(cheque_no)) {
        cy.wrap(5).should('eq', 6, { message: 'cheque number digits are same' });
      }
      cy.get('#mat-input-16').type(cheque_no);
      ...
      if (testData.category == 'others') {
        cy.get('#mat-input-17').type(testData.other_category);
        //here flag_4th_pan_letter might be A/B/J
        flag_4th_pan_letter = "A";
      }
      ...
      //------Till here code is for new cheque transactrion creation ------
      ...
      if (testData.flag_operation == 'Edit') {
        ...
        // for this transaction should be realized 1st
        cy.log('GUI selected..');
        cy.get('.mat-header-row > .cdk-column-action').click({ force: true });
        ...
        if (testData.flag_gui != 0) {
          ...
          cy.get('.mat-calendar-table').contains(today.getDate() - 1).click({ force: true });
        } else {
          cy.log('without gui');
    ...
    else if (transaction_type == 'demand-draft') {
      ...
      //submit demand-draft new transaction
      cy.get('button').contains('Submit').click({ force: true });
      ...
      cy.get('#mat-input-19').type(testData.ifcs_code);
      cy.wait(2000);
      cy.get('.bank-details', { timeout: 10000 }).click({ force: true });
      ...
      cy.wait(3000);
      // check 4th & 5th character of pan according to donor_name & selected categor
      if (testData.category == 'huf') {
        flag_4th_pan_letter = "H";
      } else if (testData.category == "partnership") {
        flag_4th_pan_letter = "F";
            ...
            if (testData.flag_category_value == 'individual') {
              ...
              if (testData.flag_proprietorship != 0) {
                ...
                if (testData.flag_proprietorship_changeTo == 'yes') {
                  ...
                  // index of space of proprietorship_name
                  let space_in_proprietorship_name = proprietorship_name.indexOf(" ");
...
describe("Action on all transactions", () => {
  ...
  it('login,perform different action on all transactions ', () => {
    cy.visit("/");
    cy.get("input[type=email").type(testData.email);
    cy.get("input[type=password").type(testData.password).type('{enter}');
    ...
    if (testData.flag_operation == 'Edit') {
      ...
      if (testData.flag_name != 0) {
        ...
        // index of space in donor name
        let space_in_name_value = name_value.indexOf(" ");
        // index of 1st letter of donor surname
        surname1st_letter = name_value.charAt(space_in_name_value + 1).toUpperCase();
        ...
        function check_Realized_Date() {
          ...
          cy.get("body").then($body => {
            if ($body.find('[formcontrolname="date_of_cheque"]').length > 0) {
              ...
              // for demand draft mode
              cy.wait(1000);
              cy.get('[formcontrolname="date_of_draft"]').invoke('val').then((myValue) => {
                cy.log("draft date:" + myValue);
                cheque_or_draft = myValue;
              })
            }
          });
