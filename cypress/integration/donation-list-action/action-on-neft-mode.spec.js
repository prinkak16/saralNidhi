...
describe("Action On NEFT Payment Mode", () => {

    let testData;
    beforeEach(function () {
        cy.fixture('donation-list-action-data/neft-action-data').then(function (data) {
            testData = data;
            return testData;
        });
    })
    ...
    it('login,actions related to neft mode payment', () => {
        cy.visit("/");
        ...
        // click on NEFT button
        cy.get('.mat-tab-label').contains('NEFT').click({ force: true });
        cy.wait(2000);
...
/*
describe("Click on Action of NEFT", () => {

    let testData;
    beforeEach(function () {
        cy.fixture('donation-list-action-data/NEFT_actionData').then(function (data) {
            testData = data;
            return testData;
        });
    })
    ...
    it('login then click on total count ', () => {
        ...
        // click on NEFT button
        cy.get('#mat-tab-label-0-3 > .mat-tab-label-content > .ng-star-inserted > .tab-text')
            .click();
            ...
            if(testData.flag_party_unit!=0)
            {
                ...
                if(testData.party_unit_value=='Mandal')
                {
                    ...
                    cy.get(':nth-child(3) > .bg-white > .ng-select-container > .ng-value-container > .ng-input > input')
                        .click().get('ng-select')
                        .contains(testData.party_unit_mandal).click();
                }
                else if(testData.party_unit_value=='Zila')
                {
                    cy.wait(3000);
