...
describe("Cheque Payment Mode", () => {
  let testData;
  beforeEach(function () {
    cy.fixture('mode-of-payments-data/cheque-mode').then(function (data) {
      testData = data;
      return testData;
    })
  })

  it('login,fill details for cheque mode payment & submit', () => {
    cy.visit("/");
    ...
    var g1_cheque_date = new Date(cheque_yyyy, cheque_mm, cheque_dd);
    ...
    console.log("**g1_cheque_date :**" + g1_cheque_date);

    if (g1_cheque_date.getTime() > end_financial_date1.getTime()) {
      cy.wrap(3).should('eq', 4, { message: 'cheque date is > this financial year date' });
    }
...
describe("Cheque Payment Mode", () => {
  ...
  it('login,fill details for cheque mode payment & submit', () => {
    ...
    if (test_same_digit(testData.cheque_no)) {
      cy.wrap(5).should('eq', 6, {message: 'cheque number digits are same'});
    }

    function cheque_digits_count(n) {
      var count = 0;
      if (n >= 1) ++count;
    ...
    //validate cheque_no
    if (cheque_digits_count(testData.cheque_no) == 6) {
      cy.get('#mat-input-16').type(testData.cheque_no);
    } else {
      cy.wrap(1).should('eq', 2, {message: 'cheque_no is not of 6 digits or not a number'});
    }
