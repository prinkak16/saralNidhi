...
public class CashRelatedTest extends Base {
	...
	public void basePageNavigationCash(ITestContext context) throws IOException, InterruptedException {
		...
		// wait.until(ExpectedConditions.textToBePresentInElement(cashPage.getWrongCollectorName(collector_name_input),"Please
		// enter a valid name"));
        ...
        log.info("submitText for Cash mode is :"+submitText);
		...
		if (submitText.contains("You can not donate more than") || submitText.contains("You can only donate")) { 
			...
			System.out.println("submitText 2nd time:" + submitText2);
			
			if(submitText2.contains("You can not donate more than") || submitText2.contains("You can only donate")) {
				log.info("You need to change pan card for new transaction creation in cash");
				System.out.println("You need to change pan card for new transaction creation in cash");
			}
			else {
				System.out.println("indide else block due to submitText2 ");
