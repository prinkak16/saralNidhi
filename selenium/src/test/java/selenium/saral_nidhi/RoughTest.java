...
public class RoughTest extends Base {
	...
	public void basePageNavigation(ITestContext context) throws IOException, InterruptedException {
		...
		String[] arr_category = { "individual" };
	...
	public void action() throws InterruptedException, IOException {
		...
		String[] actionArr = {"View","Edit","Realized","Archive"};
		...
		for(String action : actionArr) {
			...
			System.out.println("action item :"+action);
			...
			try {
				...
				WebElement actionElement = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[contains(text(),'"+action+"')]")));
				actionElement.click();	
			...
			if(action == "View") {
				...
				// 4th and 5th letter
				String error_txt_for_4th_and_5th_letter = pan_letters_category_validation_for_action(categoryChangeTo,name_of_proprietorship, copied_name, changePanNoTo,proprietorshipChangeTo);
				System.out.println("error_txt_for_4th_and_5th_letter: " + error_txt_for_4th_and_5th_letter);
				...
				if(party_unit=="State") {
				  ...
				  WebElement update = driver.findElement(By.xpath("(//*[@class='mat-simple-snackbar ng-star-inserted'])"));
				  
				  String updateText = wait.until(ExpectedConditions.visibilityOf(update)).getText();
				  
				  System.out.println("updateText :"+updateText);
				 
				  Assert.assertTrue(updateText.contains("Record Update successfully"));
			}
			else if(action == "Realized") {
				System.out.println("---Realized section---");
				WebElement realizedDateTxt = driver.findElement(By.tagName("mat-label"));
				String r = wait.until(ExpectedConditions.visibilityOf(realizedDateTxt)).getText();
				
				System.out.println("r :"+r);
				
				/*
				int no_of_back_days = 92;
				LocalDate older_than_90th_Date = now.minusDays(no_of_back_days);

				System.out.println(now);
				System.out.println("older_than_90th_Date: " + older_than_90th_Date);

				DateTimeFormatter dateFormating = DateTimeFormatter.ofPattern("dd/MM/yyyy");
				
				String ralizedDate = older_than_90th_Date.format(dateFormating);

				System.out.println("ralizedDate :"+ralizedDate);

				
				driver.findElement(By.xpath("(//*[@formcontrolname='date'])")).sendKeys(ralizedDate);
				...
				no_of_back_days = no_of_back_days+2;
				...
				System.out.println("compare dates :"+now.compareTo(ralizeDate_generate));
				
				if(now.compareTo(ralizeDate_generate) >0 ) {
					driver.findElement(By.xpath("(//*[@formcontrolname='date'])")).sendKeys(realizedDate);
					// submit button
					driver.findElement(By.xpath("//button[contains(text(),'Submit')]")).click();
					 WebElement realize = driver.findElement(By.xpath("(//*[@class='mat-simple-snackbar ng-star-inserted'])"));
	...
	private static String pan_letters_category_validation_for_action(String category_value,
