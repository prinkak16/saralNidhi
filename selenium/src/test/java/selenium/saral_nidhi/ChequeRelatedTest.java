		...
		if(party_unit=="State" || party_unit.equals("State")) {
			...
			if (party_unit == "CountryState") {
				...
				// click on cheque tab
					...
					try {
						System.out.println("from try block for item");
						WebElement actionElement = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[contains(text(),'Bounced')]")));
						actionElement.click();	
					}catch (org.openqa.selenium.StaleElementReferenceException e) {
						System.out.println("from catch block for item");
						WebElement actionElement = wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.xpath("//span[contains(text(),'Bounced')]"))));
						actionElement.click();
					}
					
					WebElement bouncedRemark = driver.findElement(By.xpath("(//*[@formcontrolname='remark'])"));
					bouncedRemark.sendKeys("ads");
