...
public class ChequeRelatedTest extends Base {
	...
	public void basePageNavigation(ITestContext context) throws IOException, InterruptedException {
		...
		filterByInstrumentNo = random_cheque_no;
		...
		// wait.until(ExpectedConditions.textToBePresentInElement(chequePage.getWrongCollectorName(collector_name_input),"Please
		// enter a valid name"));
	...
	public void realizedActionForCheque() throws InterruptedException, IOException {
		...
		if (now.compareTo(ralizeDate_generate) > 0) {
			...
			log.info("Cheque Mode Transaction Realized successfully");
			ngDriver.waitForAngularRequestsToFinish();
		}
	...
	public void reversedActionForCheque() throws InterruptedException {
		...
		log.info("Cheque Mode Reversed Successfully");
		ngDriver.waitForAngularRequestsToFinish();
	}
	...
	public void archiveActionForCheque() throws InterruptedException, IOException {
		...
		log.info("Cheque Mode Archived Successfully");
		ngDriver.waitForAngularRequestsToFinish();
	}

	@Test(dependsOnMethods = { "archiveActionForCheque" })
	...
	public void bouncedActionForCheque() throws InterruptedException, IOException {
		...
		chequePage.getIndianDonationForm().click();
		log.info("New Cheque Mode Transaction for Bounce test");
		ngDriver.waitForAngularRequestsToFinish();
		...
		if (party_unit == "CountryState") {
			...
			log.info("Cheque mode Record Saved Successfully for Bounce Test");
			...
			log.info("Cheque Mode Bounced Successfully");
			ngDriver.waitForAngularRequestsToFinish();
		}
	...
	public void unarchiveActionForCheque() throws InterruptedException, IOException {

		System.out.println("&&&& unarchiveActionForCheque section ----");
		Thread.sleep(5000);
		// explicit wait
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(50));
		...
		log.info("cheque Mode Unarchived successfully");
	}

	@Test(dependsOnMethods = { "unarchiveActionForCheque" })

	public void downloadDonationListTestForCheque() throws InterruptedException, IOException {
		// explicit wait
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(50));
		ngDriver.waitForAngularRequestsToFinish();
		Thread.sleep(5000);

		System.out.println("&&&& downloadDonationListTestForCheque section ----");
	...
	@Test(dependsOnMethods = { "downloadDonationListTestForCheque" })

	public void actionRequiredForPanCardCheque() throws InterruptedException, IOException {
		// explicit wait
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(50));
		...
		if (actionRequiredforPancardApplicable) {
			...
			System.out.println("&&& actionRequiredForPanCardCheque section ----");
