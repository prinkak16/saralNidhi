...
public class NEFTRelatedTest extends Base {
	...
	public void basePageNavigation(ITestContext context) throws IOException, InterruptedException {
		...
		random_utr_no = "neft" + random_utr_no;
		...
		collecter_phone.click();
		 ...
		 wait.until(ExpectedConditions.textToBePresentInElement(neftPage.getWrongCollectorName(collector_name_input),"Please enter a valid name"));
		//String collector_name_error = neftPage.getWrongCollectorName(collector_name_input).getText();
		...
		wait.until(ExpectedConditions.textToBePresentInElement(neftPage.getWrongCollectorPhone(collecter_phone), "Please enter correct phone number"));
		//String collector_phone_error = neftPage.getWrongCollectorPhone(collecter_phone).getText();
		...
		for (String party_unit : party_unit_arr) {
			...
			if (party_unit == "CountryState") {
				...
				zila_exist = neftPage.isElementPresent(driver, "zila").isDisplayed();
				...
				WebElement state_from_zila = wait.until(ExpectedConditions.visibilityOfElementLocated(neftPage.selectGivenState(state_unit_name)));
				
				try {
					state_from_zila.click();
				}
				catch (StaleElementReferenceException e) {
					state_from_zila = wait.until(ExpectedConditions.visibilityOfElementLocated(neftPage.selectGivenState(state_unit_name)));
					state_from_zila.click();
				}
				...
				try {
					...
					state_from_mandal = wait.until(ExpectedConditions.visibilityOfElementLocated(neftPage.selectGivenState(state_unit_name)));
					state_from_mandal.click();
				}
				...
				try {
					...
					zila_unit = wait.until(ExpectedConditions.visibilityOfElementLocated(neftPage.selectGivenZila(zila_unit_name)));
					zila_unit.click();
				}
				
				mandal_exist = neftPage.isElementPresent(driver, "mandal").isDisplayed();
		...
		// click on NEFT tab
			...
			JavascriptExecutor js = (JavascriptExecutor) driver;
	           ...
	           // clicking on 3 dots for action 
                ...
                /*
        		boolean district_bool = wait
        				.until(ExpectedConditions.textToBePresentInElementValue(neftPage.getDistrict(), "Pune"));
        		String copied_district = null;
