...
import pageObjects.NEFT_ModePage;
import resources.Base;

public class NEFTRelatedTest extends Base {
	public WebDriver driver;
	public NgWebDriver ngDriver;
	public static Logger log = LogManager.getLogger(NEFTRelatedTest.class);
	String reverseText;
	String approveText;
	String rejectText;
	...
	@Test
	public void basePageNavigationNEFT(ITestContext context) throws IOException, InterruptedException {
		...
		ArrayList<String> excel_data = dd.getData("NEFT_ModeTest", a);

		NEFT_ModePage neftPage = new NEFT_ModePage(driver);
		 ...
		 * loginButton.click();
		 */
		log.info("Login successfully in NEFT_ModeTest");

		WebElement heading1 = wait.until(ExpectedConditions.visibilityOfElementLocated(neftPage.getFirstHeading()));
		Assert.assertEquals(heading1.getText(), "Nidhi Collection");

		neftPage.getIndianDonationForm().click();
		...
		neftPage.getTransactionDate().sendKeys(transaction_Date);

		// Wrong utr no
		neftPage.getUTRNumber().sendKeys("12345 new");

		neftPage.getAccountNumber().click();

		Assert.assertEquals(neftPage.getInvalidUTR().getText(), "UTR number is invalid");

		neftPage.getUTRNumber().clear();

		Random random = new Random();
		String random_utr_no = Integer.toString(random.nextInt(900000) + 100000);

		random_utr_no = "nef" + random_utr_no;
		filterByInstrumentNo = random_utr_no;
		neftPage.getUTRNumber().sendKeys(random_utr_no);
		neftPage.getAccountNumber().sendKeys(excel_data.get(6));

		neftPage.getIfscCode().sendKeys(excel_data.get(7));

		WebElement bankdetails = wait.until(ExpectedConditions.visibilityOfElementLocated(neftPage.getbankDetails()));
		...
		neftPage.getDonorName().sendKeys(donor_name);

		String copied_name = neftPage.getDonorName().getAttribute("value");
		...
		// type wrong phone no
		neftPage.getDonorPhoneNumber().sendKeys("23222322");

		neftPage.getDonorEmail().click();

		Assert.assertEquals("Please enter a valid phone number", neftPage.getInvalidDonorPhone().getText());

		neftPage.getDonorPhoneNumber().clear();
		neftPage.getDonorPhoneNumber().sendKeys(donor_phone);

		// Enter wrong email
		neftPage.getDonorEmail().sendKeys("abc.com");

		neftPage.getHouse().click();

		Assert.assertEquals("Enter a valid email", neftPage.getInvalidDonorEmail().getText());

		neftPage.getDonorEmail().clear();
		neftPage.getDonorEmail().sendKeys(donor_email);
		neftPage.getHouse().sendKeys(excel_data.get(11));
		neftPage.getLocality().sendKeys(excel_data.get(12));
		neftPage.getPinCode().sendKeys(excel_data.get(13));

		boolean district_bool = wait
				.until(ExpectedConditions.textToBePresentInElementValue(neftPage.getDistrict(), excel_data.get(14)));
		String copied_district = null;

		if (district_bool) {
			copied_district = neftPage.getDistrict().getAttribute("value");
		...
		boolean state_bool = wait
				.until(ExpectedConditions.textToBePresentInElement(neftPage.getState(), excel_data.get(15)));

		String copied_state = null;
		if (state_bool) {
			copied_state = neftPage.getState().getText();
		...
		for (String category : arr_category) {
			...
			WebElement categoryElement = neftPage.selectCategory(category);
			...
			if (category == "individual") {
				...
				neftPage.getNoProprietorship(categoryElement).click();

				for_yes_no_proprietorship(driver, pan_no, name_of_proprietorship, copied_name, category, neftPage);

				// Now for Yes
				// Thread.sleep(1000);

				String proprietorship_txt = neftPage.getProprietorship(categoryElement).getText();
				Assert.assertEquals(proprietorship_txt, "Is it a proprietorship? *");

				WebElement proprietorship_yes = neftPage.getYesProprietorship(categoryElement);
				proprietorship_yes.click();

				// Thread.sleep(1000);

				boolean b1 = neftPage.getYesProprietorshipSelect().isSelected();

				System.out.println("proprietorship_yes is selected or not --->:" + b1);
				Assert.assertTrue(b1);

				WebElement Proprietorship_txt2 = neftPage.getTextAfterYesProprietorship(proprietorship_yes);
				...
				neftPage.getProprietorshipName().sendKeys(name_of_proprietorship);
				...
				WebElement other_category = wait
						.until(ExpectedConditions.visibilityOfElementLocated(neftPage.getOtherCategory()));
				...
				other_than_individual(driver, pan_no, name_of_proprietorship, copied_name, category, neftPage);
			}
		...
		WebElement amount_input = neftPage.getAmountInput();
		...
		String amount_in_words_txt = neftPage.getAmountInWords(amount_input).getText();
		...
		neftPage.getNarationInput().sendKeys(excel_data.get(18));
		WebElement collector_name_input = neftPage.getCollectorName();

		// collector_name_input.sendKeys("vk");
		WebElement collecter_phone = neftPage.getCollectorPhone();
		// collecter_phone.click();

		// wait.until(ExpectedConditions.textToBePresentInElement(neftPage.getWrongCollectorName(collector_name_input),"Please
		// enter a valid name"));
		...
		wait.until(ExpectedConditions.textToBePresentInElement(neftPage.getWrongCollectorPhone(collecter_phone),
		...
		for (String donation : donation_nature) {
			...
			neftPage.getDonationNature(donation).click();
			Thread.sleep(1000);
			System.out.println("nature of donation :--->" + neftPage.getDonationNature2(donation).isSelected());

			Assert.assertTrue(neftPage.getDonationNature2(donation).isSelected());

			if (donation == "Other") {
				neftPage.getOtherNatureOfDonation().sendKeys(excel_data.get(21));
			}
		...
		for (String party_unit : party_unit_arr) {
			...
			WebElement party_unit_type = neftPage.getPartyUnit(party_unit);
			party_unit_type.click();
			System.out.println(neftPage.getPartyUnit2(party_unit).isSelected());

			Assert.assertTrue(neftPage.getPartyUnit2(party_unit).isSelected());

			boolean state_exist = neftPage.isElementPresent(driver, "state").isDisplayed();
			...
			if (party_unit == "CountryState") {
				...
				WebElement selectState = wait
						.until(ExpectedConditions.visibilityOfElementLocated(neftPage.getSelectState()));

				selectState.click();

				WebElement state_unit = wait.until(
						ExpectedConditions.visibilityOfElementLocated(neftPage.selectGivenState(state_unit_name)));
				state_unit.click();
				...
				// select zila
				WebElement selectZila = wait.until(ExpectedConditions.elementToBeClickable(neftPage.getSelectZila()));
				selectZila.click();
				log.info("clicked on select zila for zila unit");

				WebElement zila_unit = wait
						.until(ExpectedConditions.visibilityOfElementLocated(neftPage.selectGivenZila(zila_unit_name)));
				zila_unit.click();
				...
				try {
					...
					zila_unit = wait.until(
							ExpectedConditions.visibilityOfElementLocated(neftPage.selectGivenZila(zila_unit_name)));
					zila_unit.click();
				}
				...
				WebElement selectMandal = wait
						.until(ExpectedConditions.elementToBeClickable(neftPage.getSelectMandal()));
				selectMandal.click();
				log.info("clicked on select mandal unit");

				WebElement mandal_unit = wait.until(
						ExpectedConditions.visibilityOfElementLocated(neftPage.selectGivenMandal(mandal_unit_name)));
				mandal_unit.click();
		...
		ngDriver.waitForAngularRequestsToFinish();
		// success
		log.info("NEFT mode Transaction Created Sucessfully");

	}

	// ------- Action Related
	@Test(dependsOnMethods = { "basePageNavigationNEFT" })
	public void viewActionForNEFT(ITestContext context) throws IOException, InterruptedException {
		...
		WebElement heading2 = wait.until(ExpectedConditions.visibilityOfElementLocated(neftPage.getFirstHeading()));
		...
		String locality = neftPage.getLocality().getAttribute("value");
		System.out.println("locality :" + locality);

		String pincode = neftPage.getPinCode().getAttribute("value");
		System.out.println("pincode :" + pincode);

		js.executeScript("window.scrollBy(0,1350)", "");

		// String district = neftPage.getDistrict().getAttribute("value");
		// System.out.println("district :"+district);

		/*
		 * boolean district_bool = wait
		 * .until(ExpectedConditions.textToBePresentInElementValue(neftPage.getDistrict(
		 * ), "Pune")); String copied_district = null;
		 * 
		 * if (district_bool) { copied_district =
		 * neftPage.getDistrict().getAttribute("value"); } else {
		 * log.error("district name didn't come till 50 sec"); }
		 * 
		 * System.out.println(copied_district);
		 * 
		 * String state = neftPage.getState().getText();
		...
		Thread.sleep(2000);
		String pan1 = neftPage.get1stPanInput().getAttribute("value");
		String pan2 = neftPage.get2ndPanInput().getAttribute("value");
		String pan3 = neftPage.get3rdPanInput().getAttribute("value");
		String pan4 = neftPage.get4thPanInput().getAttribute("value");

		String pan5 = neftPage.get5thPanInput().getAttribute("value");
		String pan6 = neftPage.get6thPanInput().getAttribute("value");
		String pan7 = neftPage.get7thPanInput().getAttribute("value");
		String pan8 = neftPage.get8thPanInput().getAttribute("value");

		String pan9 = neftPage.get9thPanInput().getAttribute("value");
		String pan10 = neftPage.getLastPanInput().getAttribute("value");
	...
	public void editActionForNEFT() throws InterruptedException, IOException {
		...
		js.executeScript("window.scrollBy(0,750)", "");
		// neftPage.uploadFrontImage();
		// neftPage.uploadBackImage();

		neftPage.getAccountNumber().clear();
		neftPage.getAccountNumber().sendKeys(excel_data.get(6));

		neftPage.getIfscCode().clear();
		neftPage.getIfscCode().sendKeys(excel_data.get(4));
		...
		neftPage.getHouse().clear();
		neftPage.getHouse().sendKeys(excel_data.get(8));
		neftPage.getLocality().clear();
		neftPage.getLocality().sendKeys(excel_data.get(9));
		neftPage.getPinCode().clear();
		neftPage.getPinCode().sendKeys(excel_data.get(10));

		boolean district_bool = wait
				.until(ExpectedConditions.textToBePresentInElementValue(neftPage.getDistrict(), excel_data.get(11)));
		String copied_district = null;
		...
		boolean state_bool = wait
				.until(ExpectedConditions.textToBePresentInElement(neftPage.getState(), excel_data.get(12)));
		...
		WebElement categoryElement = neftPage.selectCategory(categoryChangeTo);
		...
		if (categoryChangeTo == "individual" || categoryChangeTo.equals("individual")) {
			if (proprietorshipChangeTo == "yes" || proprietorshipChangeTo.equals("yes")) {
				...
				Assert.assertEquals(Proprietorship_txt2.getText(), "Write the name of the Proprietorship");
				// type Name of proprietorship
				neftPage.getProprietorshipName().clear();
				neftPage.getProprietorshipName().sendKeys(name_of_proprietorship);
			} else {
				WebElement proprietorship_no = neftPage.getNoProprietorship(categoryElement);
		...
		neftPage.get1stPanInput().clear();
		neftPage.get1stPanInput().sendKeys(changePanNoTo[0]);
		neftPage.get2ndPanInput().sendKeys(changePanNoTo[1]);
		neftPage.get3rdPanInput().sendKeys(changePanNoTo[2]);
		neftPage.get4thPanInput().sendKeys(changePanNoTo[3]);
		neftPage.get5thPanInput().sendKeys(changePanNoTo[4]);
		neftPage.get6thPanInput().sendKeys(changePanNoTo[5]);
		neftPage.get7thPanInput().sendKeys(changePanNoTo[6]);
		neftPage.get8thPanInput().sendKeys(changePanNoTo[7]);
		neftPage.get9thPanInput().sendKeys(changePanNoTo[8]);
		neftPage.getLastPanInput().sendKeys(changePanNoTo[9]);

		// last pan input
		WebElement last_pan_input = neftPage.getLastPanInput();

		// invalid pan no
		String text_for_invalid_pan = pan_letters_varification(driver, neftPage);
		...
		if (text_for_invalid_pan != "") {
			...
			WebElement pan_error_paragraph = neftPage.getPanErrorParagraph(last_pan_input);
			System.out.println("Assertion for text_for_invalid_pan pass....");

			neftPage.uploadWrongPanImage();
			neftPage.getWrongPanRemark().clear();
			neftPage.getWrongPanRemark().sendKeys("text_for_invalid_pan");
			...
			neftPage.uploadWrongPanImage();
			neftPage.getWrongPanRemark().clear();
			neftPage.getWrongPanRemark().sendKeys("error_txt_for_4th_and_5th_letter");
		...
		Assert.assertEquals(amount_converted_in_words, amount_in_words_txt);
		neftPage.getNarationInput().clear();
		neftPage.getNarationInput().sendKeys(excel_data.get(19));
		...
		neftPage.getDonationNature(donationNature).click();
		// Assert.assertTrue(neftPage.getDonationNature2(donationNature).isSelected());

		if (donationNature == "Other" || donationNature.equals("Other")) {
			neftPage.getOtherNatureOfDonation().clear();
			neftPage.getOtherNatureOfDonation().sendKeys(excel_data.get(23));
		}
		...
		if (party_unit == "CountryState") {
			...
			WebElement state_unit = wait
					.until(ExpectedConditions.visibilityOfElementLocated(neftPage.selectGivenState(state_unit_name)));
			state_unit.click();
			log.info(state_unit_name + " selected");
			...
			// select mandal
			WebElement selectMandal = wait.until(ExpectedConditions.elementToBeClickable(neftPage.getSelectMandal()));
			selectMandal.click();

			log.info("clicked on select mandal");
			WebElement mandal_unit = wait
					.until(ExpectedConditions.visibilityOfElementLocated(neftPage.selectGivenMandal(mandal_unit_name)));
			mandal_unit.click();
			log.info(mandal_unit_name + " selected for mandal unit");
		}
		...
		log.info("NEFT Mode Transaction edited successfully");
		// setting reverseText
	...
	public void reversedActionForNEFT() throws InterruptedException, IOException {
		...
		log.info("NEFT Mode Reversed Successfully");
		ngDriver.waitForAngularRequestsToFinish();
	...
	public void archiveActionForNEFT() throws InterruptedException, IOException {
		...
		log.info("NEFT Transaction Archived Successfully");
	}

	@Test(dependsOnMethods = { "archiveActionForNEFT" })
	public void unarchiveActionForNEFT() throws InterruptedException, IOException {

		System.out.println("&&&& unarchiveActionForNEFT section ----");
		Thread.sleep(5000);
		// explicit wait
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(50));
		...
		log.info("NEFT Transaction Unarchive successfully");
	}

	@Test(dependsOnMethods = { "unarchiveActionForNEFT" })

	public void downloadDonationListTestForNEFT() throws InterruptedException, IOException {
		// explicit wait
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(50));
		ngDriver.waitForAngularRequestsToFinish();
		Thread.sleep(5000);

		System.out.println("&&&& downloadDonationListTestForNEFT section ----");
		...
		ArrayList<String> excel_data_for_download_total_form = dd.getData("NEFT", a);
	...
	@Test(dependsOnMethods = { "downloadDonationListTestForNEFT" })

	public void actionRequiredForPanCardNEFT() throws InterruptedException, IOException {
		// if pan card was correct in edit then following block will be executed
		if (actionRequiredforPancardApplicable) {

			System.out.println("&&& actionRequiredForPanCardNEFT section ----");
	...
	private static String pan_letters_varification(WebDriver driver, NEFT_ModePage neftPage) {

		List<WebElement> elements = neftPage.getPanElements();
		String whole_pan = "";
	...
	private static void for_yes_no_proprietorship(WebDriver driver, String[] pan_no, String name_of_proprietorship,
			String copied_name, String category, NEFT_ModePage neftPage) {
		...
		for (int i = 0; i < 5; i++) {
			...
			fill_pan_number(driver, pan_no, neftPage);
			...
			String error_txt_for_4th_and_5th_letter = pan_letters_category_validation(driver, category,
					name_of_proprietorship, copied_name, neftPage);
			System.out.println("error_txt_for_4th_and_5th_letter:" + error_txt_for_4th_and_5th_letter);
	...
	private static void fill_pan_number(WebDriver driver, String[] pan, NEFT_ModePage neftPage) {

		neftPage.get1stPanInput().clear();
		neftPage.get1stPanInput().sendKeys(pan[0]);
		neftPage.get2ndPanInput().sendKeys(pan[1]);
		neftPage.get3rdPanInput().sendKeys(pan[2]);
		neftPage.get4thPanInput().sendKeys(pan[3]);
		neftPage.get5thPanInput().sendKeys(pan[4]);
		neftPage.get6thPanInput().sendKeys(pan[5]);
		neftPage.get7thPanInput().sendKeys(pan[6]);
		neftPage.get8thPanInput().sendKeys(pan[7]);
		neftPage.get9thPanInput().sendKeys(pan[8]);
		neftPage.getLastPanInput().sendKeys(pan[9]);
	...
	private static String pan_letters_category_validation(WebDriver driver, String category_value,
			String name_of_proprietorship, String copied_name, NEFT_ModePage neftPage) {

		List<WebElement> pan_elements = neftPage.getPanElements();
		String mismatch_error = "";
