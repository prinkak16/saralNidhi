...
public class DemandDraftRelatedTest extends Base {
	...
	@Test
	public void basePageNavigationDemandDraft(ITestContext context) throws IOException, InterruptedException {
		...
		filterByInstrumentNo = random_draft_no;
		...
		// wait.until(ExpectedConditions.textToBePresentInElement(demandDraftPage.getWrongCollectorName(collector_name_input),
		...
		log.info("DemadDraft mode Transaction Created Successfully");
	...
	@Test(dependsOnMethods = { "basePageNavigationDemandDraft" })
	...
	public void editActionForDemandDraft() throws InterruptedException, IOException {
		...
		log.info("Demand Draft Mode Transaction Edited Successfully");
	...
	public void realizedActionForDemandDraft() throws InterruptedException, IOException {
		...
		if (now.compareTo(ralizeDate_generate) > 0) {
			...
			log.info("Demand Draft Realized Successfully");
			ngDriver.waitForAngularRequestsToFinish();
		}
	}
	...
	public void reversedActionForDemandDraft() throws InterruptedException {
		...
		log.info("Demand Draft Mode Reversed Successfully");
		ngDriver.waitForAngularRequestsToFinish();
	}
	...
	public void archiveActionForDemandDraft() throws InterruptedException, IOException {
		...
		log.info("Demand Draft Transaction Archived Successfully");
	}

	@Test(dependsOnMethods = { "archiveActionForDemandDraft" })
	public void unarchiveActionForDemandDraft() throws InterruptedException, IOException {

		System.out.println("&&&& unarchiveActionForDD section ----");
		Thread.sleep(5000);
		// explicit wait
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(50));
		...
		log.info("Demand Draft Transaction Unarchive successfully");
	}

	@Test(dependsOnMethods = { "unarchiveActionForDemandDraft" })

	public void downloadDonationListTestForDemandDraft() throws InterruptedException, IOException {
		// explicit wait
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(50));
		ngDriver.waitForAngularRequestsToFinish();
		Thread.sleep(5000);

		System.out.println("&&&& downloadDonationListTestForDemandDraft section ----");
		...
		ArrayList<String> excel_data_for_download_total_form = dd.getData("DemandDraft", a);
	...
	@Test(dependsOnMethods = { "downloadDonationListTestForDemandDraft" })

	public void actionRequiredForPanCardDemandDraft() throws InterruptedException, IOException {
		// if pan card was correct in edit then following block will be executed
		if (actionRequiredforPancardApplicable) {

			System.out.println("&&& actionRequiredForPanCardDemandDraft section ----");
			...
			WebElement downloadBtn = wait.until(ExpectedConditions
					.elementToBeClickable(driver.findElement(By.xpath("//button[contains(text(),'Download')]"))));
			((JavascriptExecutor) driver).executeScript("arguments[0].click();", downloadBtn);
			ngDriver.waitForAngularRequestsToFinish();
			Thread.sleep(4000);
			...
			for (File listOfFile : listOfFiles) {
				if (listOfFile.isFile()) {
					fileName = listOfFile.getName();
					System.out.println("fileName " + fileName);
					if (fileName.contains("ActionRequiredForPancard")) {
						found = true;
						log.info("downloaded file :" + fileName);
					}
				}
