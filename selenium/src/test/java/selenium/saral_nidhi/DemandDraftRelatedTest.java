...
public class DemandDraftRelatedTest extends Base {
	...
	public void basePageNavigation(ITestContext context) throws IOException, InterruptedException {
		...
		boolean district_bool = wait
				.until(ExpectedConditions.textToBePresentInElementValue(demandDraftPage.getDistrict(), excel_data.get(14)));
		...
		for (String category : arr_category) {
			...
			if (category == "individual") {
				...
				for_yes_no_proprietorship(driver, pan_no, name_of_proprietorship, copied_name, category, demandDraftPage);
	...
	public void viewActionForDemandDraft(ITestContext context) throws IOException, InterruptedException {
		...
		WebElement heading1 = wait.until(ExpectedConditions.visibilityOfElementLocated(demandDraftPage.getFirstHeading()));
		Assert.assertEquals(heading1.getText(), "Nidhi Collection");
		...
		WebElement heading2 = wait.until(ExpectedConditions.visibilityOfElementLocated(demandDraftPage.getFirstHeading()));
		...
		// click on Demand Draft tab
			...
			JavascriptExecutor js = (JavascriptExecutor) driver;
	           ...
	           // clicking on 3 dots for action 
                ...
                /*
        		boolean district_bool = wait
        				.until(ExpectedConditions.textToBePresentInElementValue(demandDraftPage.getDistrict(), "Pune"));
        		String copied_district = null;
	...
	try {
		...
		//demandDraftPage.getAccountNumber().sendKeys(excel_data.get(6));
		demandDraftPage.getAccountNumber().sendKeys("123456789101112000");

		demandDraftPage.getIfscCode().clear();
		demandDraftPage.getIfscCode().sendKeys(excel_data.get(4));
		
		WebElement bankdetails = wait.until(ExpectedConditions.visibilityOfElementLocated(demandDraftPage.getbankDetails()));
		...
		boolean district_bool = wait
				.until(ExpectedConditions.textToBePresentInElementValue(demandDraftPage.getDistrict(), excel_data.get(11)));
		String copied_district = null;
	...
	try {
		System.out.println("from try block for item");
		WebElement actionElement = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[contains(text(),'Realized')]")));
		actionElement.click();	
	}catch (org.openqa.selenium.StaleElementReferenceException e) {
		System.out.println("from catch block for item");
		WebElement actionElement = wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.xpath("//span[contains(text(),'Realized')]"))));
		actionElement.click();
	}
		...
		WebElement allDate = driver.findElement(By.xpath("(//*[@class='mat-cell cdk-cell cdk-column-date mat-column-date ng-star-inserted'])["+rowNo+"]"));
