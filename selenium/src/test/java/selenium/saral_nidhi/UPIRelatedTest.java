...
public class UPIRelatedTest extends Base {
	public WebDriver driver;
	public NgWebDriver ngDriver;
	public static Logger log = LogManager.getLogger(UPIRelatedTest.class);
	...
	@Test
	public void basePageNavigationUPI(ITestContext context) throws IOException, InterruptedException {
		...
		ArrayList<String> excel_data = dd.getData("UPI_ModeTest", a);

		
		UPI_ModePage upiPage = new UPI_ModePage(driver);
		// explicit wait
        ...
        */
		...
		upiPage.getIndianDonationForm().click();
		ngDriver.waitForAngularRequestsToFinish();

		WebElement paymentModeOption = wait
				.until(ExpectedConditions.visibilityOfElementLocated(upiPage.getUPIPaymentMode()));

		// upi should not be selected
		Assert.assertFalse(upiPage.getUPIModeValue().isSelected());

		paymentModeOption.click();
		ngDriver.waitForAngularRequestsToFinish();

		// upi should be selected
		Assert.assertTrue(upiPage.getUPIModeValue().isSelected());
		log.info("UPI Mode of Payment selected");
		...
		upiPage.getTransactionDate().sendKeys(transaction_Date);

		// Wrong utr no
		upiPage.getUTRNumber().sendKeys("12345 new");

		upiPage.getAccountNumber().click();

		Assert.assertEquals(upiPage.getInvalidUTR().getText(), "UTR number is invalid");

		upiPage.getUTRNumber().clear();

		Random random = new Random();
		String random_utr_no = Integer.toString(random.nextInt(900000) + 100000);

		random_utr_no = "upi" + random_utr_no;
		filterByInstrumentNo = random_utr_no;
		upiPage.getUTRNumber().sendKeys(random_utr_no);
		upiPage.getAccountNumber().sendKeys(excel_data.get(6));

		upiPage.getIfscCode().sendKeys(excel_data.get(7));

		WebElement bankdetails = wait.until(ExpectedConditions.visibilityOfElementLocated(upiPage.getbankDetails()));
		...
		upiPage.getDonorName().sendKeys(donor_name);

		String copied_name = upiPage.getDonorName().getAttribute("value");
		...
		// type wrong phone no
		upiPage.getDonorPhoneNumber().sendKeys("23222322");

		upiPage.getDonorEmail().click();

		Assert.assertEquals("Please enter a valid phone number", upiPage.getInvalidDonorPhone().getText());

		upiPage.getDonorPhoneNumber().clear();
		upiPage.getDonorPhoneNumber().sendKeys(donor_phone);

		// Enter wrong email
		upiPage.getDonorEmail().sendKeys("abc.com");

		upiPage.getHouse().click();

		Assert.assertEquals("Enter a valid email", upiPage.getInvalidDonorEmail().getText());

		upiPage.getDonorEmail().clear();
		upiPage.getDonorEmail().sendKeys(donor_email);
		upiPage.getHouse().sendKeys(excel_data.get(11));
		upiPage.getLocality().sendKeys(excel_data.get(12));
		upiPage.getPinCode().sendKeys(excel_data.get(13));

		boolean district_bool = wait
				.until(ExpectedConditions.textToBePresentInElementValue(upiPage.getDistrict(), excel_data.get(14)));
		String copied_district = null;

		if (district_bool) {
			copied_district = upiPage.getDistrict().getAttribute("value");
		} else {
		...
		boolean state_bool = wait
				.until(ExpectedConditions.textToBePresentInElement(upiPage.getState(), excel_data.get(15)));

		String copied_state = null;
		if (state_bool) {
			copied_state = upiPage.getState().getText();
		} else {
		...
		for (String category : arr_category) {
			...
			WebElement categoryElement = upiPage.selectCategory(category);
			...
			if (category == "individual") {
				...
				upiPage.getNoProprietorship(categoryElement).click();

				for_yes_no_proprietorship(driver, pan_no, name_of_proprietorship, copied_name, category, upiPage);

				String proprietorship_txt = upiPage.getProprietorship(categoryElement).getText();
				Assert.assertEquals(proprietorship_txt, "Is it a proprietorship? *");

				WebElement proprietorship_yes = upiPage.getYesProprietorship(categoryElement);
				proprietorship_yes.click();

				// Thread.sleep(1000);

				boolean b1 = upiPage.getYesProprietorshipSelect().isSelected();

				System.out.println("proprietorship_yes is selected or not --->:" + b1);
				Assert.assertTrue(b1);

				WebElement Proprietorship_txt2 = upiPage.getTextAfterYesProprietorship(proprietorship_yes);
				...
				upiPage.getProprietorshipName().sendKeys(name_of_proprietorship);
				...
				WebElement other_category = wait
						.until(ExpectedConditions.visibilityOfElementLocated(upiPage.getOtherCategory()));
				...
				other_than_individual(driver, pan_no, name_of_proprietorship, copied_name, category, upiPage);
			}
		...
		WebElement amount_input = upiPage.getAmountInput();
		...
		String amount_in_words_txt = upiPage.getAmountInWords(amount_input).getText();
		...
		upiPage.getNarationInput().sendKeys(excel_data.get(18));
		WebElement collector_name_input = upiPage.getCollectorName();

		collector_name_input.sendKeys("vk");
		WebElement collecter_phone = upiPage.getCollectorPhone();
		collecter_phone.click();
		wait.until(ExpectedConditions.textToBePresentInElement(upiPage.getWrongCollectorName(collector_name_input),
		...
		wait.until(ExpectedConditions.textToBePresentInElement(upiPage.getWrongCollectorPhone(collecter_phone),
		...
		for (String donation : donation_nature) {
			...
			upiPage.getDonationNature(donation).click();
			Thread.sleep(1000);
			System.out.println("nature of donation :--->" + upiPage.getDonationNature2(donation).isSelected());

			Assert.assertTrue(upiPage.getDonationNature2(donation).isSelected());

			if (donation == "Other") {
				upiPage.getOtherNatureOfDonation().sendKeys(excel_data.get(21));
			}
		...
		for (String party_unit : party_unit_arr) {
			...
			WebElement party_unit_type = upiPage.getPartyUnit(party_unit);
			party_unit_type.click();
			System.out.println(upiPage.getPartyUnit2(party_unit).isSelected());

			Assert.assertTrue(upiPage.getPartyUnit2(party_unit).isSelected());

			boolean state_exist = upiPage.isElementPresent(driver, "state").isDisplayed();
			...
			if (party_unit == "CountryState") {
				...
				WebElement zila_unit = wait
						.until(ExpectedConditions.visibilityOfElementLocated(upiPage.selectGivenZila(zila_unit_name)));
				zila_unit.click();
				...
				WebElement selectMandal = wait
						.until(ExpectedConditions.elementToBeClickable(upiPage.getSelectMandal()));
				selectMandal.click();
				log.info("clicked on select mandal unit");
				
				WebElement mandal_unit = wait.until(
						ExpectedConditions.visibilityOfElementLocated(upiPage.selectGivenMandal(mandal_unit_name)));
				mandal_unit.click();
	...
	// ------------- Action Related
	@Test(dependsOnMethods = { "basePageNavigationUPI" })
	public void viewActionForUPI(ITestContext context) throws IOException, InterruptedException {

		ArrayList<String> a = new ArrayList<String>();
		JavascriptExecutor js = (JavascriptExecutor) driver;
		ActionReletedDataDriven dd = new ActionReletedDataDriven();
		ArrayList<String> excel_data = dd.getData("upiMode", a);
		...
		WebElement heading2 = wait.until(ExpectedConditions.visibilityOfElementLocated(upiPage.getFirstHeading()));
		...
		// click on UPI tab
		WebElement paymentMode = driver.findElement(By.xpath("(//*[@class='tab-text'])[7]"));
		paymentMode.click();
		log.info("View section");
		Thread.sleep(2000);
		...
		String locality = upiPage.getLocality().getAttribute("value");
		System.out.println("locality :" + locality);

		String pincode = upiPage.getPinCode().getAttribute("value");
		System.out.println("pincode :" + pincode);

		js.executeScript("window.scrollBy(0,1350)", "");

		// String district = upiPage.getDistrict().getAttribute("value");
		// System.out.println("district :"+district);

		/*
		 * boolean district_bool = wait
		 * .until(ExpectedConditions.textToBePresentInElementValue(upiPage.getDistrict()
		 * , "Pune")); String copied_district = null;
		 * 
		 * if (district_bool) { copied_district =
		 * upiPage.getDistrict().getAttribute("value"); } else {
		 * log.error("district name didn't come till 50 sec"); }
		 * 
		 * System.out.println(copied_district);
		 * 
		 * String state = upiPage.getState().getText();
		...
		Thread.sleep(2000);
		String pan1 = upiPage.get1stPanInput().getAttribute("value");
		String pan2 = upiPage.get2ndPanInput().getAttribute("value");
		String pan3 = upiPage.get3rdPanInput().getAttribute("value");
		String pan4 = upiPage.get4thPanInput().getAttribute("value");

		String pan5 = upiPage.get5thPanInput().getAttribute("value");
		String pan6 = upiPage.get6thPanInput().getAttribute("value");
		String pan7 = upiPage.get7thPanInput().getAttribute("value");
		String pan8 = upiPage.get8thPanInput().getAttribute("value");

		String pan9 = upiPage.get9thPanInput().getAttribute("value");
		String pan10 = upiPage.getLastPanInput().getAttribute("value");
	...
	@Test(dependsOnMethods = { "viewActionForUPI" })

	public void editActionForUPI() throws InterruptedException, IOException {
		...
		js.executeScript("window.scrollBy(0,750)", "");
		// upiPage.uploadFrontImage();
		// upiPage.uploadBackImage();

		upiPage.getAccountNumber().clear();
		upiPage.getAccountNumber().sendKeys(excel_data.get(6));

		upiPage.getIfscCode().clear();
		upiPage.getIfscCode().sendKeys(excel_data.get(4));
		...
		upiPage.getHouse().clear();
		upiPage.getHouse().sendKeys(excel_data.get(8));
		upiPage.getLocality().clear();
		upiPage.getLocality().sendKeys(excel_data.get(9));
		upiPage.getPinCode().clear();
		upiPage.getPinCode().sendKeys(excel_data.get(10));

		boolean district_bool = wait
				.until(ExpectedConditions.textToBePresentInElementValue(upiPage.getDistrict(), excel_data.get(11)));
		String copied_district = null;
		...
		boolean state_bool = wait
				.until(ExpectedConditions.textToBePresentInElement(upiPage.getState(), excel_data.get(12)));
		...
		WebElement categoryElement = upiPage.selectCategory(categoryChangeTo);
		...
		if (categoryChangeTo == "individual" || categoryChangeTo.equals("individual")) {
			if (proprietorshipChangeTo == "yes" || proprietorshipChangeTo.equals("yes")) {
				...
				Assert.assertEquals(Proprietorship_txt2.getText(), "Write the name of the Proprietorship");
				// type Name of proprietorship
				upiPage.getProprietorshipName().clear();
				upiPage.getProprietorshipName().sendKeys(name_of_proprietorship);
			} else {
				WebElement proprietorship_no = upiPage.getNoProprietorship(categoryElement);
		...
		upiPage.get1stPanInput().clear();
		upiPage.get1stPanInput().sendKeys(changePanNoTo[0]);
		upiPage.get2ndPanInput().sendKeys(changePanNoTo[1]);
		upiPage.get3rdPanInput().sendKeys(changePanNoTo[2]);
		upiPage.get4thPanInput().sendKeys(changePanNoTo[3]);
		upiPage.get5thPanInput().sendKeys(changePanNoTo[4]);
		upiPage.get6thPanInput().sendKeys(changePanNoTo[5]);
		upiPage.get7thPanInput().sendKeys(changePanNoTo[6]);
		upiPage.get8thPanInput().sendKeys(changePanNoTo[7]);
		upiPage.get9thPanInput().sendKeys(changePanNoTo[8]);
		upiPage.getLastPanInput().sendKeys(changePanNoTo[9]);

		// last pan input
		WebElement last_pan_input = upiPage.getLastPanInput();

		// invalid pan no
		String text_for_invalid_pan = pan_letters_varification(driver, upiPage);
		...
		if (text_for_invalid_pan != "") {
			...
			WebElement pan_error_paragraph = upiPage.getPanErrorParagraph(last_pan_input);
			System.out.println("Assertion for text_for_invalid_pan pass....");

			upiPage.uploadWrongPanImage();
			upiPage.getWrongPanRemark().clear();
			upiPage.getWrongPanRemark().sendKeys("text_for_invalid_pan");
			...
			upiPage.uploadWrongPanImage();
			upiPage.getWrongPanRemark().clear();
			upiPage.getWrongPanRemark().sendKeys("error_txt_for_4th_and_5th_letter");
		...
		Assert.assertEquals(amount_converted_in_words, amount_in_words_txt);
		upiPage.getNarationInput().clear();
		upiPage.getNarationInput().sendKeys(excel_data.get(19));
		...
		upiPage.getDonationNature(donationNature).click();
		// Assert.assertTrue(upiPage.getDonationNature2(donationNature).isSelected());

		if (donationNature == "Other" || donationNature.equals("Other")) {
			upiPage.getOtherNatureOfDonation().clear();
			upiPage.getOtherNatureOfDonation().sendKeys(excel_data.get(23));
		}
		...
		if (party_unit == "CountryState") {
			...
			WebElement state_unit = wait
					.until(ExpectedConditions.visibilityOfElementLocated(upiPage.selectGivenState(state_unit_name)));
			state_unit.click();
			log.info(state_unit_name + " selected");
			...
			// select mandal
			WebElement selectMandal = wait.until(ExpectedConditions.elementToBeClickable(upiPage.getSelectMandal()));
			selectMandal.click();
			log.info("clicked on select mandal");
			
			WebElement mandal_unit = wait
					.until(ExpectedConditions.visibilityOfElementLocated(upiPage.selectGivenMandal(mandal_unit_name)));
			mandal_unit.click();
			log.info(mandal_unit_name + " selected for mandal unit");
		}
		...
		log.info("UPI Mode Transaction edited successfully");
		// setting reverseText
	...
	@Test(dependsOnMethods = { "editActionForUPI" })
	public void reversedActionForUPI() throws InterruptedException, IOException {
		...
		log.info("UPI Mode Reversed Successfully");
	}

	@Test(dependsOnMethods = { "reversedActionForUPI" })
	public void archiveActionForUPI() throws InterruptedException, IOException {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(50));
		ngDriver.waitForAngularRequestsToFinish();
		Thread.sleep(5000);
		...
		log.info("UPI Transaction Archived Successfully");
	}

	@Test(dependsOnMethods = { "archiveActionForUPI" })
	public void unarchiveActionForUPI() throws InterruptedException, IOException {

		System.out.println("&&&& unarchiveActionForUPI section ----");
		Thread.sleep(5000);
		// explicit wait
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(50));
		...
		log.info("UPI Transaction Unarchive successfully");
	}

	@Test(dependsOnMethods = { "unarchiveActionForUPI" })

	public void downloadDonationListTestForUPI() throws InterruptedException, IOException {
		// explicit wait
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(50));
		ngDriver.waitForAngularRequestsToFinish();
		Thread.sleep(5000);

		System.out.println("&&&& downloadDonationListTestForUPI section ----");
		...
		ArrayList<String> excel_data_for_download_total_form = dd.getData("UPI", a);
	...
	@Test(dependsOnMethods = { "downloadDonationListTestForUPI" })

	public void actionRequiredForPanCardUPI() throws InterruptedException, IOException {
		// if pan card was correct in edit then following block will be executed
		if (actionRequiredforPancardApplicable) {

			System.out.println("&&& actionRequiredForPanCardUPI section ----");
	...
	private static String pan_letters_varification(WebDriver driver, UPI_ModePage upiPage) {

		List<WebElement> elements = upiPage.getPanElements();
		String whole_pan = "";
	...
	private static void for_yes_no_proprietorship(WebDriver driver, String[] pan_no, String name_of_proprietorship,
			String copied_name, String category, UPI_ModePage upiPage) {
		...
		for (int i = 0; i < 5; i++) {
			...
			fill_pan_number(driver, pan_no, upiPage);
			...
			String error_txt_for_4th_and_5th_letter = pan_letters_category_validation(driver, category,
					name_of_proprietorship, copied_name, upiPage);
			System.out.println("error_txt_for_4th_and_5th_letter:" + error_txt_for_4th_and_5th_letter);
	...
	private static void fill_pan_number(WebDriver driver, String[] pan, UPI_ModePage upiPage) {

		upiPage.get1stPanInput().clear();
		upiPage.get1stPanInput().sendKeys(pan[0]);
		upiPage.get2ndPanInput().sendKeys(pan[1]);
		upiPage.get3rdPanInput().sendKeys(pan[2]);
		upiPage.get4thPanInput().sendKeys(pan[3]);
		upiPage.get5thPanInput().sendKeys(pan[4]);
		upiPage.get6thPanInput().sendKeys(pan[5]);
		upiPage.get7thPanInput().sendKeys(pan[6]);
		upiPage.get8thPanInput().sendKeys(pan[7]);
		upiPage.get9thPanInput().sendKeys(pan[8]);
		upiPage.getLastPanInput().sendKeys(pan[9]);
	...
	private static String pan_letters_category_validation(WebDriver driver, String category_value,
			String name_of_proprietorship, String copied_name, UPI_ModePage upiPage) {

		List<WebElement> pan_elements = upiPage.getPanElements();
		String mismatch_error = "";
